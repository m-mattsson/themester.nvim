local filesystem = require("themester.filesystem")
local constants = require("themester.constants")

local state_folder_path = vim.fn.stdpath("data") .. "/themester"
local state_file_path = state_folder_path .. "/state.json"

-- Configuration settings for Themester plugin
...
local function isConfigValid()
	...
	if #errors > 0 then
		print("Themester config has some errors in their config:")
...
-- Sets up the Themester configuration with user-defined settings
...
--- Retrieves the paths used by Themester
